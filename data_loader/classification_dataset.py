...
class ClassificationDataset(data.Dataset):
    def __init__(
        ...
        if os.path.exists(data_split_path):
            ...
            self.valid_idx = len(self.train_indices) + np.asarray(
    ...
    def preprocess(self, idx):
        ...
        x_tl = seq[n]["text_{}_len".format(self.text)] // 512 + int(
            (seq[n]["text_{}_len".format(self.text)] % 512) == 0
        )
        x_tl = torch.Tensor(
            [
                x_tl,
    ...
    def preprocess_non_seq(self, idx):
        ...
        if n > 1:
            ...
            pcode = (
                len(self.diag_vocab) * self.diag + np.asarray(s["procedures"])
            ) * self.proc
            mcode = (
                len(self.diag_vocab) * self.diag
                + len(self.proc_vocab) * self.proc
        ...
        if self.sg_text:
            x_text = self.sg_model.get_sentence_vector(
                v["text_{}_raw".format(self.text)]
            )
