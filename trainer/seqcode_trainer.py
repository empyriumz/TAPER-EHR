...
class SeqCodeTrainer(BaseTrainer):
    ...
    def __init__(
        ...
        super(SeqCodeTrainer, self).__init__(
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, (x, m, ivec, jvec) in enumerate(self.data_loader):
            ...
            target = (
                target.transpose(1, 0).contiguous().view(-1, target.size(-1)).float()
            )
            ...
            loss = (
                loss_dict["visit_loss"] * self.visit_loss
                + loss_dict["code_loss"] * self.code_loss
            )

            loss.backward()
            torch.nn.utils.clip_grad_norm_(
                self.model.parameters(), self.config.get("clip", 0.25)
            )
            ...
            total_metrics += self._eval_metrics(
                logits.detach(), target.detach(), mask=mask.bool().detach()
            )  # , k=self.config['trainer']['recall_k'])
