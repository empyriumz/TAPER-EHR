...
logging = logging.getLogger(
    "model.bert_things.pytorch_pretrained_bert.tokenization"
...
def embed_text(text_codes, device, bert, bert_seq_length=512, max_seq_len_text=30):
    ...
    for i in range(len(text_codes) // bert_seq_length):
        x_text[:, i] = torch.Tensor(
            text_codes[i * bert_seq_length : (1 + i) * bert_seq_length]
        )
        x_mask[:, i] = 1
    if n * bert_seq_length <= len(text_codes):
        x_mask[len(text_codes) - bert_seq_length * (n + 1), n] = 1
        x_text[: len(text_codes) - bert_seq_length * (n + 1), n] = torch.Tensor(
            text_codes[(n + 1) * bert_seq_length :]
        )
...
def main():
    ...
    parser.add_argument(
        "-cpb", "--bert_config_path", default=None, type=str, help="path to bert config"
    )
    parser.add_argument(
        "-vpb", "--bert_vocab_path", default=None, type=str, help="path to bert vocab "
    )
    parser.add_argument(
        "-sdp",
        "--state_dict_path",
    ...
    parser.add_argument(
        ...
        default=0,
        type=int,
        help="pass this if maximum text sequence length is known for discharge text to avoid long processing time",
    )
    parser.add_argument(
        "-tslr",
        ...
        default=0,
        type=int,
        help="pass this if maximum text sequence length is known for rest of text (other than discharge) to avoid longer processing time",
    )
    parser.add_argument(
        "-sc",
        "--short_code",
    ...
    df.loc[:, "MARITAL_STATUS"], demographic_cols["MARITAL_STATUS"] = pd.factorize(
        df["MARITAL_STATUS"]
    )
    ...
    (
        df.loc[:, "DISCHARGE_LOCATION"],
        demographic_cols["DISCHARGE_LOCATION"],
    ) = pd.factorize(df["DISCHARGE_LOCATION"])
    df.loc[:, "LAST_CAREUNIT"], demographic_cols["LAST_CAREUNIT"] = pd.factorize(
        df["LAST_CAREUNIT"]
    )
    ...
    try:
        for pid in tqdm(pids):
            ...
            for i, r in pid_df.iterrows():
                ...
                dtok, ptok, mtok, ctok = [], [], [], []
                diagnosis_codes, proc_codes, med_codes, cpt_codes = (
                    np.nan,
                    np.nan,
                    np.nan,
                    np.nan,
                ...
                if args.medications:
                    med_codes = r[
                        "NDC"
                    ]  # issue with NDC what mapping version is being used..?
