...
if __name__ == "__main__":
    ...
    df_adm_notes["ADMITTIME_C"] = df_adm_notes.ADMITTIME.apply(
        lambda x: str(x).split(" ")[0]
    )
    df_adm_notes["ADMITTIME_C"] = pd.to_datetime(
        df_adm_notes.ADMITTIME_C, format="%Y-%m-%d", errors="coerce"
    )
    df_adm_notes["CHARTDATE"] = pd.to_datetime(
        df_adm_notes.CHARTDATE, format="%Y-%m-%d", errors="coerce"
    )
