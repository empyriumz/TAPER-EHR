...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=1,
        type=int,
        help="numpy seed used to create datasplit",
    )
    parser.add_argument("-k", "--kfold", default=7, type=int, help="kfold split")
    parser.add_argument(
        "-filter_codes",
        action="store_true",
        help="filter codes based on frequency count",
    )
    parser.add_argument(
        "-min_adm",
        type=int,
        help="min number of admissions filter, af must be specified",
    )
    ...
    if args.filter_codes:
        ...
        for k, v in data_data.items():
            ...
            for i in range(len(v)):
                v[i]["procedures"] = list(
                    ret_filtered_code(v[i]["procedures"], proc_codes)
                )
                v[i]["diagnoses"] = list(
                    ret_filtered_code(v[i]["diagnoses"], diag_codes)
                )
                v[i]["medications"] = list(
                    ret_filtered_code(v[i]["medications"], med_codes)
                )
        data_temp = {}
        ...
        try:
            with open(
                os.path.abspath(os.path.join(args.save, "..", "data_filtered.pkl")),
                "wb",
            ) as handle:
    ...
    for idx, ids in enumerate(kf.split(pids)):
        ...
        pickle.dump(
            ids, open(os.path.join(args.save, "split_{}.pkl".format(idx)), "wb")
        )
