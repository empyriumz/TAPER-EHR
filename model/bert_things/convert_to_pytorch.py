...
parser.add_argument(
    "--path", default=".", type=str, help="path to tensorflow checkpoint"
)
parser.add_argument(
    "--config", default="./bert_config.json", type=str, help="path to bert config file"
)
parser.add_argument(
    "--save", default="./pytorch_weights", type=str, help="path to save file"
)
...
def convert_tf_checkpoint_to_pytorch(
    ...
    init_vars = list(
        filter(
            lambda x: all([True if e not in x[0] else False for e in excluded]),
            init_vars,
        )
    )
    names = []
