...
def bce_loss_with_logits(output, target, weight=[1.0, 1.0], pos_weight = 1.0):
    weight = ((target == 1).float() * weight[1]) + ((target == 0).float() * weight[0])
...
def med2vec_loss_transformer(
    ...
    def visit_loss(x, mask, probits, window=1):
        ...
        for i in range(0, window):
            ...
            tl = bce_loss(forward_preds, forward_xi) + bce_loss(
                backward_preds, backward_xi
            )
...
def med2vec_loss(
    ...
    def visit_loss(x, mask, probits, window=1):
        ...
        for i in range(0, window):
            ...
            tl = bce_loss(forward_preds, x[i + 1 :].float()) + bce_loss(
                backward_preds, x[: -i - 1].float()
            )
            if not torch.isnan(tl):
                loss += tl
